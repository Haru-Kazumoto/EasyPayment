<div class="flex flex-col gap-3">
    ...
    <div class="flex justify-between items-center p-2">
        <div class="flex flex-col gap-2">
            <span class="text-2xl font-semibold">Daftar Kelas</span>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-medium text-blue-600 md:ms-2">Daftar Kelas</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5 flex flex-col gap-4" action="<?= url('classes/store') ?>" method="POST">
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            ...
                            <div class="col-span-2">
                                <label for="code" class="block mb-2 text-sm font-medium text-gray-900">Kode Kelas</label>
                ...
                <tbody>
                    <?php if (empty($classes)): ?>
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td colspan="8" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-2 font-medium">Tidak ada data kelas</p>
                                <p class="text-xs">Silakan tambahkan kelas baru</p>
                            </td>
                        </tr>
                    <?php endif; ?>
                    <?php foreach ($classes as $index => $class): ?>
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-100 text-gray-900 font-medium whitespace-nowrap">
                            ...
                            <td class="px-6 py-4">
                                <?= format_date($class['created_at']) ?>
                            </td>
                            <td class="px-6 py-4 flex gap-4">
                                <button onclick="editClass(<?= $class['id'] ?>)" type="button" class="text-yellow-600 hover:text-yellow-800 hover:bg-yellow-50 p-2 rounded-lg transition-colors">
                                ...
                                <button data-modal-target="delete-modal"
                                    ...
                                    data-id="<?= $class['id'] ?>"
                                    data-name="<?= htmlspecialchars($class['name']) ?>" type="button" class="delete-btn text-red-600 hover:text-red-800 hover:bg-red-50 p-2 rounded-lg transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            ...
            <div id="edit-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    ...
                    <div class="relative bg-white rounded-lg shadow-sm">
                        ...
                        <!-- Modal body -->
                        <form id="editClassesForm" class="p-4 md:p-5 flex flex-col gap-4">
                            <input type="hidden" name="edit_classes_id" id="edit_classes_id">

                            <div class="grid gap-4 mb-4 grid-cols-2">
                                <div class="col-span-2">
                                    <label for="edit_name" class="block mb-2 text-sm font-medium text-gray-900">Nama Kelas</label>
                                    <input type="text" name="edit_name" id="edit_name"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                        placeholder="Tagihan Bulanan" required>
                                </div>
                                <div class="col-span-2">
                                    <label for="edit_code" class="block mb-2 text-sm font-medium text-gray-900">Kode Kelas</label>
                                    <input type="text" name="edit_code" id="edit_code"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                        placeholder="SPP" required>

                                    <script>
                                        const editCodeInput = document.getElementById('code');
                ...
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow-sm">
                        ...
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <!-- Form untuk DELETE -->
                            <form id="delete-form" method="POST" action="<?= url('classes/delete') ?>">
...
<script>
    const BASE_URL = '<?= url("classes/detail") ?>';
    let modal;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('editClassesForm').addEventListener('submit', handleSubmit);
    });
    ...
    async function editClass(id) {
        try {
            const url = `${BASE_URL}&class_id=${id}`;
    ...
    function populateForm(data) {
        document.getElementById('edit_classes_id').value = data.id || '';
        document.getElementById('edit_name').value = data.name || '';
        document.getElementById('edit_code').value = data.code || '';
    }

    function clearForm() {
        document.getElementById('edit_classes_id').value = '';
    ...
    async function handleSubmit(e) {
        ...
        try {
            const url = '<?= url('classes/update') ?>';
            const formData = new FormData(e.target);

            const response = await fetch(`${url}&class_id=${document.getElementById('edit_classes_id').value}`, {
                method: 'POST',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                },
